...
const findChallengeByName = (challengeName) => {
    for (const c in datacache_1.challenges) {
        if (Object.prototype.hasOwnProperty.call(datacache_1.challenges, c)) {
            if (datacache_1.challenges[c].name === challengeName) {
                return datacache_1.challenges[c];
            }
        }
    }
...
const findChallengeById = (challengeId) => {
    for (const c in datacache_1.challenges) {
        if (Object.prototype.hasOwnProperty.call(datacache_1.challenges, c)) {
            if (datacache_1.challenges[c].id === challengeId) {
                return datacache_1.challenges[c];
            }
        }
    }
