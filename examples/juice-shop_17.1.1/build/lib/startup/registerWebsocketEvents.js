...
const registerWebsocketEvents = (server) => {
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('verifySvgInjectionChallenge', (data) => {
            challengeUtils.solveIf(datacache_1.challenges.svgInjectionChallenge, () => { return data?.match(/.*\.\.\/\.\.\/\.\.[\w/-]*?\/redirect\?to=https?:\/\/placekitten.com\/(g\/)?[\d]+\/[\d]+.*/) && security.isRedirectAllowed(data); });
        });
